...
def model_compare():
    ...
    splits = get_splits(y, valid_size=.2, test_size=0.1, stratify=True, random_state=ranxxx, shuffle=True,
                        show_plot=False)
...
def show_two_model():
    ...
    fft_learn = load_learner_all(path='models', dls_fname='fft_dls', model_fname='fft_model',
                                 learner_fname='fft_learner')
    dls = fft_learn.dls
    valid_dl = dls.valid
...
def dbn_pre(x_3d, fft_x_3d, y, ):
    splits = get_splits(y, valid_size=.2, test_size=0.1, stratify=True, random_state=ranyyy, shuffle=True,
                        show_plot=False)
    tfms = [None, [Categorize()]]
    bs = 64
    ...
    dbn_x_train = np.zeros(
        (x_probas_train_array.shape[0], x_probas_train_array.shape[1] + fft_probas_train_array.shape[1]))
    dbn_x_train[:, 0:x_probas_train_array.shape[1]] = x_probas_train_array
    dbn_x_train[:, x_probas_train_array.shape[1]:x_probas_train_array.shape[1] + fft_probas_train_array.shape[
        1]] = fft_probas_train_array
    ...
    dbn_x_valid = np.zeros(
        (x_probas_valid_array.shape[0], x_probas_valid_array.shape[1] + fft_probas_valid_array.shape[1]))
    dbn_x_valid[:, 0:x_probas_valid_array.shape[1]] = x_probas_valid_array
    dbn_x_valid[:, x_probas_valid_array.shape[1]:x_probas_valid_array.shape[1] + fft_probas_valid_array.shape[
        1]] = fft_probas_valid_array
...
def my_dbn_train(dbn_x_train, y_train, dbn_x_valid, y_valid):
    # 创建 PyTorch 数据集
...
def dbn_show():
    progress = pd.read_csv('DBN_with_pretraining_classifier.csv')
    fig, ax = plt.subplots()
    ax.plot(progress['epochs'], progress['train loss'], label='Train Loss')
    ax.plot(progress['epochs'], progress['test loss'], label='Validation Loss')
    ...
    fig, ax = plt.subplots()
    ax.plot(progress['epochs'], progress['train acc'], label='Train Accuracy')
    ax.plot(progress['epochs'], progress['test acc'], label='Validation Accuracy')
...
# x_learn,fft_learn=show_two_model()
