...
class RBM:
	...
	def __init__(self, n_visible, n_hidden,Option:option):
		self.mode = option.rbm_opt.mode # bernoulli or gaussian RBM
		self.n_hidden = n_hidden #  Number of hidden nodes
		self.n_visible = n_visible # Number of visible nodes
		self.lr = option.rbm_opt.learning_rate # Learning rate for the CD algorithm
		self.epochs = option.rbm_opt.epochs # Number of iterations to run the algorithm for
		self.batch_size = option.rbm_opt.batch_size
		self.k = option.rbm_opt.k
		self.optimizer = option.rbm_opt.optimizer
		self.beta_1=0.9
		self.beta_2=0.999
		self.epsilon=1e-07
		self.m = [0, 0, 0]
		...
		self.savefile = option.rbm_opt.savefile
		self.early_stopping_patience = option.rbm_opt.early_stopping_patience
		self.stagnation = 0
		self.previous_loss_before_stagnation = 0
		self.progress = []

		if torch.cuda.is_available() and option.rbm_opt.gpu==True:  
			dev = "cuda:0" 
		else:  
			dev = "cpu"  
